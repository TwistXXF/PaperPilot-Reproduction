...
def stage_encode():
    ...
    for ds in ['scidocs', 'scifact']:
        ...
        for s in range(0, len(docs), CHUNK):
            fp = os.path.join(out_dir, f'chunk_{s}.npy')
            part = docs[s:s + CHUNK]
            ids += [str(d['_id']) for d in part]
...
def prep(ds):
    ...
    np.save(os.path.join(out, 'lsa.npy'), L)
    np.save(os.path.join(out, 'C.npy'), C)
    np.save(os.path.join(out, 'R.npy'), R)
    np.save(os.path.join(out, 'V.npy'), V)
    np.save(os.path.join(out, 'lens.npy'), lens)
    json.dump(doc_ids, open(os.path.join(out, 'doc_ids.json'), 'w'))
    joblib.dump(bm25, os.path.join(out, 'bm25.joblib'))
    joblib.dump(vec, os.path.join(out, 'tfidf.joblib'))
    joblib.dump(svd, os.path.join(out, 'svd.joblib'))
    return out
...
if __name__ == '__main__':
    ...
    if stage == 'all':
        ...
        if len(sys.argv) > 2 and stage in ('retrieval', 'ablation', 'robust',
                                           'router'):
