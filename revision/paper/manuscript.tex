...
\author{Zichen Feng\\
Department of Computer Science and Technology, University of Sanya\\
Sanya, Hainan, China\\
...
pre-specified citation/recency rerankings.  Before materialising a locked-test label file
or computing locked metrics, we publicly froze the split, action family, metric
definitions, calibration rule, and stopping criterion.  The pre-committed experiment
uses \dataset, an expert-curated biomedical
...
\begin{enumerate}
  \item A publicly timestamped protocol and phase-separated
  \code{fit}, \code{calibrate}, \code{freeze}, and \code{evaluate} outputs.  Frozen
  decisions and all score/provenance inputs are hashed before a locked-test label file
  or locked metric can be created.
  \item A new pre-committed evaluation on human graded relevance rather than a
...
contains more than 180,000 document-pair judgements supplied by over 1,500 scientists
...
levels and its fixed per-query candidate pools of 53--60 documents.
...
SciNCL, and the SPECTER2 proximity adapter on exactly the same candidate pools; the
BGE checkpoint belongs to the C-Pack embedding resources \citep{xiao2024cpack}.
Scholarly retrieval remains sensitive to linguistic form, while aspect-level
multi-vector representations can improve fine-grained document similarity
\citep{singh2022inefficiency,mysore2022multivector}.

Recent systems span fine-grained abstract analysis, sequential writing assistance,
large citation graphs, and local citation-context modelling
\citep{takahashi2022solutiontailor,santosa2024s3par,stergiopoulos2024academic,
kanwal2024multiple,jebari2023context,goyal2024symtax}.  Knowledge-aware citation
recommendation and OAG-Bench represent, respectively, a local citation task and
human-curated academic-graph tasks \citep{wu2024supporting,zhang2024oagbench}; their
scores are not directly comparable with RELISH seed-paper relatedness.  Their breadth
nevertheless makes a supervised fusion baseline more informative than comparison only
...
Formal post-selection risk guarantees require accounting for parameter selection, for
example through learn-then-test calibration \citep{angelopoulos2025learn}; the selected
Clopper--Pearson screen used here is not such a guarantee.
...
The SHA-256 of the salt \code{relish-v1} and group title assigns buckets 0--1 to training, bucket 2 to
...
The primary metric is NDCG@10 with linear graded gain, matching the convention used
by SciRepEval's \code{trec\_eval} evaluator \citep{jarvelin2002}.  Secondary
...
trapezoidal area \citep{zhou2025aurc}.
...
\IfFileExists{generated/table_matched.tex}{
  \input{generated/table_matched.tex}
}{
  \begin{center}\fbox{Matched-coverage table is generated only from the completed locked run.}\end{center}
}
...
A
verification command independently recomputes every numeric section of the result file,
checks all frozen score/provenance hashes, and regenerates the LaTeX tables.  A v3.2
erratum records metric, preprocessing, label-container, and integrity corrections made
before any locked metric was computed.  The core v3.0 protocol commit predates the
label download; however, an early development extractor subsequently loaded the public
all-split Parquet before filtering its output.  It wrote no locked-label file and no
locked result was inspected, but this prevents a literal ``labels were never read'' or
external-blinding claim.  Version 3.2 instead Arrow-filters split rows before Python
materialisation, and the freeze manifest records specifically that locked labels and
metrics were not supplied to decision fitting.
